    ...
    <div id="admin-content" role="tabpanel">
        ...
        <div class="tab-content">
            ...
            <div role="tabpanel" class="tab-pane" id="security">
                ...
                <div class="form-group">
                    <label for="ip_ban_list" class="control-label">List of IP networks to ban in CIDR notation (one value per line)</label>
...
$oneclick_update_success = <<<TEMPLATE
<p class="alert alert-success">
    <span class="glyphicon glyphicon-ok"></span>
...
function suspicious($server, $comment) {
    if($server) {
        @file_get_contents($server."report/".urlencode(getIP())."/".urlencode($comment));
    }
}
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($r = $q->fetch()) {
            if(stripos(file_get_contents('templates/'.$r[0].'/index.php'), 'libs/mmc.js') !== FALSE) {
                if(!empty($_POST["address"]) || !empty($_POST["honeypot"])) {
    ...
    if(!$disable_admin_panel && array_key_exists('p', $_GET) && $_GET['p'] == 'admin') {
        ...
        if(array_key_exists("$session_prefix-logged_in", $_SESSION)) { // logged in to admin page
            ...
            if (array_key_exists('task', $_POST) && $_POST['task'] == 'oneclick-update') {
                ...
                rrmdir($dir);
                header("Location: ?p=admin&update_status=success");
                die();
            }
            ...
            if (
                ...
                if ($_GET["update_status"] == "success") {
                    $oneclick_update_alert = $oneclick_update_success;
                } else {
                    $oneclick_update_alert = $oneclick_update_fail;
                }
            } else {
                $oneclick_update_alert = "";
            }
